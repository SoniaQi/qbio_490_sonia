...
```{r setup}
    knitr::opts_knit$set(root.dir = normalizePath("/Users/soniaqi/Documents/QBio-490/qbio_490_sonia/week8_transcriptomics")) 
```
...
```{r}
# Read in csv files
rna_counts <- read.csv("/Users/soniaqi/Documents/QBio-490/qbio_490_sonia/analysis_data/brca_rna_count_data.csv")
rna_genes <- read.csv("/Users/soniaqi/Documents/QBio-490/qbio_490_sonia/analysis_data/brca_clinical_data.csv")
rna_clinical <- read.csv("/Users/soniaqi/Documents/QBio-490/qbio_490_sonia/analysis_data/brca_rna_clinical_data.csv")
...
```{r}
na_mask <-  ifelse(is.na(rna_clinical$age_category)|is.na(rna_clinical$ajcc_pathologic_stage)|is.na(rna_clinical$race), F, T)
...
genename_mask <- ifelse(rna_genes$gene_name %in% results$gene_name, T, F) # my gene id is messed up, so I used gene names
...
rownames(down_reg_results) <- down_reg_results$gene_id # I switched to gene_id (actually row# in results) because using gene name gives me "duplicate 'row.names'" error
```
...
Gene ensembl id: ENSG00000198889 (something is wrong with my id, so I find the corresponding id from another df)
...
The full name of the gene is DCAF12L1, and it's responsible for encoding a member of the WD repeat protein family, which help facilitate a variety of cellular processes, including cell cycle progression, signal transduction, apoptosis, and gene regulation. 

Gene ensembl id: ENSG00000201910
Log2FoldChange: -5.394198
p-adj value: 1.856387e-01
More highly expressed in young or old patients: old
Google the gene. What is its full name, and what appears to be its function? Genecards generally has a pretty good summary:
The full name is RNU1-140P. It's a pseudogene, meaning that it's an imperfect copy of a functioning gene.
...
head(up_reg_results)
head(down_reg_results)
write.csv(up_reg_results,"/Users/soniaqi/Documents/QBio-490/qbio_490_sonia/analysis_data/brca_up_reg_data.csv", row.names = FALSE)
write.csv(down_reg_results,"/Users/soniaqi/Documents/QBio-490/qbio_490_sonia/analysis_data/brca_down_reg_data.csv", row.names = FALSE)
```
...
top-right genes: up-regulated, significant
bottom-right genes: up-regulated, not significant
top-left genes: down-regulated, significant
bottom-left genes: down-regulated, not significant
top-middle genes: approx. equal expression, significant
bottom-middle genes: approx. equal expression, not significant
...
EnhancedVolcano(results,
    ...
    x = "log2foldchange",
    y = "neglog10padj") #error: attempt to select less than one element in get1index <real>
...
EnhancedVolcano(results,
    ...
    FCcutoff = 1.3)
    #x = results[,3],
    #y = results[,6]) #error: attempt to select less than one element in get1index <real>
ggsave("/Users/soniaqi/Documents/QBio-490/qbio_490_sonia/week8_transcriptomics/volcanoplot.jpg")
```
