...
```{r}
write.csv(clinic, "/Users/soniaqi/Documents/QBio-490/qbio_490_sonia/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/Users/soniaqi/Documents/QBio-490/qbio_490_sonia/analysis_data/brca_clinical_data.csv")
```
...
head(clinic$gender)
# 2. string
clinic[5,]$age_at_initial_pathologic_diagnosis
# 3. 50
clinic[1,]$age_at_initial_pathologic_diagnosis+clinic[1,]$days_to_last_followup/365
# 66 years old
```
...
```{r Exercise 4.1}
plot(clinic$age_at_initial_pathologic_diagnosis,clinic$lymph_node_examined_count,xlab="age",ylab="lymph node count",main="age vs. lymph node count")
```
...
```{r Exercise 4.2}
boxplot(clinic$age_at_initial_pathologic_diagnosis~clinic$gender,xlab="gender",ylab="age",main="age vs. gender")
```
...
```{r}
under30_over60_mask <- ifelse(clinic$age_at_initial_pathologic_diagnosis<30|clinic$age_at_initial_pathologic_diagnosis>60,T,F)
# I used age below 35 years old because that seems to be the actual purpose based on the naming
```
